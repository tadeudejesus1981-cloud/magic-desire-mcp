...
// ==========================================
server.tool('calculate_funnel_and_roi', 'Calcula a taxa de conversão do funil de checkout e o ROI puro do Produto Âncora baseado nos event_logs.', {
    ...
    try {
        ...
        funnelData?.forEach(log => {
            if (log.event_action === 'add_to_cart')
                sessionsWithCart.add(log.session_id);
            if (log.event_action === 'payment_success')
                sessionsWithPayment.add(log.session_id);
        });
